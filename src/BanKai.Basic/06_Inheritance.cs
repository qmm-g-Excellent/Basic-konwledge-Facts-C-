...
namespace BanKai.Basic
{
    ...
    public class Inheritance
    {
        ...
        {
            var demoClass = new PolymorphismDemoClass();
            var castToBaseClass = demoClass as PolymorphismDemoClassBase;//as类型转换，比较安全


           // Console.WriteLine(castToBaseClass);
        ...
        public void should_return_null_if_it_is_not_castable()
        {
            ...
            var castResult = castToObject as StringBuilder;//as类型转换失败时，结果为null

            // Console.WriteLine(castResult+"pppppppp");
        ...
        public void should_throw_exception_if_it_is_not_castable()
        {
            ...
            // please change the variable value to fix the test.
            Type expectedExceptionType = typeof(InvalidCastException);//下面的强制类型转换失败，所以就会抛出无效异常InvalidCastException
        ...
        public void should_reference_to_same_object_after_casting()
        {
            ...
            bool referenceEqual = ReferenceEquals(demoClass, castToBaseClass);//比较地址是否相同，类型转换不会改变引用（地址）
